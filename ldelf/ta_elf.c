 ...
 * queue. For compatibility with TAs which are statically linked against libutee
 * version 3.9.0 and which have initialization/finalization function.
 */
...
static TEE_Result realloc_elf_phdr_info(vaddr_t va, size_t cnt, bool is_32bit)
{
	...
	struct _dl_phdr_info32 *dlpi32 = NULL;
			       ...
			       bool is_32bit)
{
	...
	if (is_32bit) {
		...
		dlpi32 = (struct _dl_phdr_info32 *)(vaddr_t)info32->dlpi + idx;
		...
		dlpi->dlpi_subs = 0; /* No unloading on dlclode() currently */
...
TEE_Result ta_elf_set_elf_phdr_info(bool is_32bit)
{
	...
	res = ta_elf_resolve_sym("__elf_phdr_info", &info_va, NULL);
	if (res) {
		if (res == TEE_ERROR_ITEM_NOT_FOUND) {
			/* Older TA */
			return TEE_SUCCESS;
