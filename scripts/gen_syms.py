...
# Usage: nm -v tee.elf | gen_syms.py > syms_data.c
#
...
def make_symbol(line):
     s = line.split(" ")
     name = s[2].strip()
     return Symbol(int(s[0], 16), s[1], name, name.encode('ascii'))

def compress_names(symbols):
    # (De)compression table
    table = {}

    # This algorightm can be improved. But even right now it allows 50%
    # compression.
    #
    # Idea is to build substitution table and store there common substrings
    # like "__" or "tee_".
    #
    # Table will have exactly 256 entries, so every byte in compressed symbol
    # name corresponds to table enty. To eliminate corner cases, table will
    ...
    # less are used in OP-TEE there will be plenty empty holes in table.
    #
...
def emit_data(symbols, tok_table):
    ...
    print("const vaddr_t syms_offset = {};".format(offset))

    print("const char syms_token_table[] = {")
    for i in range(256):
          print("\t" + (",".join(map(lambda x: "'{}'".format(chr(x)), tok_table[i]))) + ",")
    print("};")
    ...
    for s in symbols:
        print("\t{{{off}, {name_off}, {name_len}}},".format(off = s.addr - offset,
                                                    name_off = name_off,
                                                    name_len = len(s.name)))
